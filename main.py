...
def load_llm():
    ...
    if llm_loaded:
        logging.info("LLM already loaded.")
        return True
    try:
        ...
        llm_loaded = True
        logging.info("Phi-3.1-mini-128k-instruct model loaded successfully.")
        return True
    except Exception as e:
...
def unload_llm():
    ...
    if not llm_loaded or llm is None:
        logging.info("LLM not loaded, nothing to unload.")
        return True
    try:
        logging.info("Attempting to unload LLM...")
        del llm
        llm = None
        llm_loaded = False
        gc.collect()
        logging.info("LLM unloaded successfully and garbage collected.")
        return True
    except Exception as e:
        logging.error(f"Failed to unload LLM: {e}", exc_info=True)
        llm = None
        llm_loaded = False
        return False
        ...
        try:
            ...
            if not ocr_result or not ocr_result[0]:
                ...
                if self.improve_translation:
                    improved_text = self.improve_translation_with_llm(original_text, translated_text)
...
class ControlWindow(QMainWindow):
    ...
    def toggleImproveTranslation(self):
        ...
        if self.improve_translation_enabled and not llm_loaded:
            QMessageBox.warning(self, "Warning", "LLM not loaded. Please load it from Settings > Load LLM.")
    ...
    def loadLLMAction(self):
        if load_llm():
            QMessageBox.information(self, "Success", "LLM loaded successfully.")
        else:
            QMessageBox.critical(self, "Error", f"Failed to load LLM. Check logs and ensure {MODEL_PATH} exists.")

    def unloadLLMAction(self):
        if unload_llm():
            QMessageBox.information(self, "Success", "LLM unloaded successfully.")
        else:
            QMessageBox.critical(self, "Error", "Failed to unload LLM. Check logs for details.")
...
class CaptureWidget(QWidget):
    ...
    def saveCapture(self):
        if not self.current_capture_path:
            QMessageBox.warning(self, "Warning", "No capture to save.")
            return
        save_path, _ = QFileDialog.getSaveFileName(self, "Save Capture", "", "PNG Files (*.png)")
        if save_path:
            shutil.copy(self.current_capture_path, save_path)

    def saveTranslation(self):
        if not self.current_translation_path:
            QMessageBox.warning(self, "Warning", "No translation to save.")
            return
        save_path, _ = QFileDialog.getSaveFileName(self, "Save Translation", "", "PNG Files (*.png)")
        if save_path:
            shutil.copy(self.current_translation_path, save_path)
...
class ChatWindow(QDialog):
    ...
    def start_streaming_response(self, message):
        if not llm or not llm_loaded:
            self.chat_history.append("AI: Error: LLM not loaded. Please load the model from Settings > Load LLM.")
            self.send_btn.setEnabled(True)
            return
        self.worker = LLMStreamingWorker(message, self.original_text, self.translated_text, self.parent.source_language, self.parent.target_language)
...
class LLMStreamingWorker(QThread):
    ...
    def run(self):
        if not self.is_running or not llm or not llm_loaded:
            self.finished.emit("Error: LLM not loaded.")
            return
        try:
            ...
            response = llm(prompt, max_tokens=150, temperature=0.7, top_p=0.95, stream=True)
            full_response = ""
