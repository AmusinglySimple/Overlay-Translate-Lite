...
os.environ["KMP_DUPLICATE_LIB_OK"] = "TRUE" # Keep if needed for other libs

# Qt Imports
from PySide6 import QtCore, QtWidgets, QtGui
from PySide6.QtWidgets import QApplication, QMessageBox, QSplashScreen, QSystemTrayIcon # Added QSplashScreen, QSystemTrayIcon
...
# --- MODIFIED: Import Tesseract configuration ---
from utils.ocr_utils import configure_tesseract
# --- END MODIFIED ---
...
if __name__ == '__main__':
    ...
    if splash_pix:
        ...
        # --- MODIFIED: Update splash message for Tesseract ---
        splash.showMessage("Configuring Tesseract OCR...", Qt.AlignBottom | Qt.AlignCenter, Qt.white)
        # --- END MODIFIED ---
        app.processEvents()

    # --- Configure Tesseract OCR Engine ---
    logger.info("Configuring Tesseract OCR Engine...")
    ocr_config_success = configure_tesseract() # Call the new configuration function

    if splash: # Update splash after OCR config
        # --- MODIFIED: Update splash message for Tesseract ---
        status_msg = "Tesseract Configured." if ocr_config_success else "Tesseract Config Failed!"
        splash.showMessage(status_msg, Qt.AlignBottom | Qt.AlignCenter, Qt.white)
        # --- END MODIFIED ---
        app.processEvents()
        time.sleep(0.5 if ocr_config_success else 1.5) # Show status briefly
        splash.close()

    if not ocr_config_success:
         logger.critical("Tesseract OCR engine failed to configure during startup.")
         # QMessageBox shown by configure_tesseract
         sys.exit(1)
    logger.info("Tesseract OCR configuration complete.")
