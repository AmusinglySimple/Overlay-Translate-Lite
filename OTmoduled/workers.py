...
class TranslationWorker(QThread):
    ...
    def preprocess_image(self, image):
        # (Keep the existing preprocess_image method here)
        try:
    ...
    def run(self):
        ...
        temp_file = None
        ocr = get_ocr_instance() # Get the initialized instance
        ...
        try:
            ...
            try:
                ...
                logger.debug(f"Preprocessed image saved to: {temp_file}")
            except Exception as e:
                raise Exception(f"Failed to save preprocessed image: {e}")

            logger.debug(f"Starting PaddleOCR on: {temp_file}")
            ocr_start_time = time.time()
            try:
                ocr_result = ocr.ocr(temp_file, cls=True)
                logger.debug(f"PaddleOCR finished in {time.time() - ocr_start_time:.3f} seconds.")
            except RuntimeError as ocr_runtime_err:
                logger.error(f"PaddleOCR runtime error: {ocr_runtime_err}", exc_info=True)
                raise Exception(f"OCR engine runtime error: {ocr_runtime_err}")
            except Exception as e:
                logger.error(f"PaddleOCR processing failed: {e}", exc_info=True)
                raise Exception(f"OCR engine failed: {e}")
            ...
            if not ocr_result or not ocr_result[0]:
                logger.warning("OCR detected no text.")
                ...
                for line_info in ocr_result[0]:
                    if len(line_info) >= 2 and len(line_info[0]) >= 4 and len(line_info[1]) >= 2:
                        ...
                        logger.warning(f"Unexpected OCR result structure skipped: {line_info}")

                if not lines:
                    logger.warning("OCR result structure valid, but no lines extracted.")
                    ...
                    logger.info(f"OCR successful: Found {len(lines)} lines. Avg Confidence: {avg_confidence:.2f}")
                    ...
                    if actual_source_lang == 'auto':
                        try:
                            if len(original_text.strip()) > 5:
                                text_to_detect = original_text[:1000]
                                ...
                                logger.info("Text too short for auto-detection, keeping 'auto'.")
                        except Exception as lang_detect_err:
                            logger.warning(f"Language detection failed: {lang_detect_err}. Falling back to English ('en').")
                    ...
                    if translated_text is None:
                        ...
                        # Use the detected or specified actual_source_lang
...
class AITranslationWorker(QThread):
    # (Keep the existing AITranslationWorker class here)
    # ... (copy the entire class from otfull2.py) ...
    # Make sure imports for keyring, re, requests, json, etc., are present at the top of workers.py
    # Update logging to use 'logger' from config
    # Ensure ai_api_config is imported from config
    translation_complete = Signal(str)
    error = Signal(str)

    # Corrected __init__ signature to match TranslationWorker's call
    def __init__(self, text, source_language, target_language, num_lines, parent=None):
        super().__init__(parent)
        # Removed unused self.message, self.target_lang
        self.text = text
        self.source_language = source_language
        self.target_language = target_language
    ...
    def run(self):
        ...
        try:
            ...
            source_lang_name = lang_map.get(self.source_language, self.source_language)
            ...
            if provider == "OpenAI":
                ...
                if endpoint.endswith('/api/chat'):
                     data = { "model": "llama3.2:1b", "messages": [{"role": "user", "content": prompt}],
                ...
                data = { "model": "loaded-model", "messages": [{"role": "user", "content": prompt}],
...
class AIStreamingWorker(QThread):
    # (Keep the existing AIStreamingWorker class here)
    # ... (copy the entire class from otfull2.py) ...
    # Make sure imports for keyring, re, requests, json, time are present
    ...
    def run(self):
        ...
        try:
            lang_map = { "en": "English", "es": "Spanish", "fr": "French", "de": "German",
                        ...
                        "ja": "Japanese", "ko": "Korean", "ch": "Chinese" }
            target_lang_name = lang_map.get(self.target_language, self.target_language)
