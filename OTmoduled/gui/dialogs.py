...
class TranslatedImageViewer(QDialog):
    ...
    def __init__(self, image_path, boxes, translated_lines, initial_font_size, target_language_code, control_window_ref, parent=None):
        ...
        # Get the current settings from the control window reference first
        vs = {}
        if self.control_window_ref and hasattr(self.control_window_ref, 'last_viewer_settings'):
            # *** Fetch the LATEST settings from control window ***
            vs = self.control_window_ref.last_viewer_settings
            if vs and isinstance(vs, dict): # Check if it's a non-empty dictionary
                logger.debug("Loaded viewer settings from ControlWindow reference.")
            else: # If empty dict or None, fallback to loading file
        ...
        else: # Fallback if no control window ref
             settings_from_file = load_settings()
             vs = settings_from_file.get('viewer_settings', {})
        ...
        # Safely get tuple values, handling list or tuple from load ## REVISED ##
        ...
        if isinstance(font_color_val, (list, tuple)) and len(font_color_val) == 4:
            font_color_tuple = tuple(font_color_val) # Ensure it's a tuple
        else:
            font_color_tuple = default_font_color_tuple # Fallback

        bg_outer_val = vs.get('bg_color_outer', default_bg_outer_tuple)
        if isinstance(bg_outer_val, (list, tuple)) and len(bg_outer_val) == 4:
            bg_outer_tuple = tuple(bg_outer_val) # Ensure it's a tuple
        else:
            bg_outer_tuple = default_bg_outer_tuple # Fallback
        ...
        # Derive inner BG color based on outer
    ...
    def initUI(self):
        ...
        # Color Pickers using QGridLayout for alignment
    ...
    def updateFont(self, font_display_name):
        ...
        if new_path and os.path.exists(new_path):
            if self.font_path != new_path:
             ...
             if font_display_name in self.font_family_to_path and os.path.exists(self.font_family_to_path[font_display_name]):
                 logger.debug(f"Keeping font path: {self.font_family_to_path[font_display_name]}")
                 self.font_path = self.font_family_to_path[font_display_name]
             else:
                 logger.warning(f"Could not find valid path for font '{font_display_name}'. Keeping previous: {self.font_path}")
    ...
    def resetStyles(self):
        ...
        default_font_color_tuple = (0, 0, 255, 255)
        default_bg_outer_tuple = (0, 0, 0, 180)
        self.font_color = QColor(*default_font_color_tuple)
        self.bg_color_outer = QColor(*default_bg_outer_tuple)
        h, s, v, a = self.bg_color_outer.getHsvF()
        ...
        if self.control_window_ref and hasattr(self.control_window_ref, 'default_font_size'):
            parent_initial_size = self.control_window_ref.default_font_size
        self.font_path = get_system_font_path(self.target_language_code)
    ...
    def renderTranslatedImage(self):
        ...
        try:
            font = ImageFont.truetype(self.font_path, self.font_size)
             ...
             logger.warning(f"Failed to load font '{self.font_path}' size {self.font_size}. Falling back.")
             fallback_path = get_system_font_path("default")
             try: font = ImageFont.truetype(fallback_path, self.font_size)
             except (IOError, TypeError):
                 logger.error("Failed to load even default fallback font. Using PIL default.")
                 font = ImageFont.load_default()
                 self.font_path = None
             if self.font_path is not None: self.font_path = fallback_path
        ...
        if not self.boxes:
            try:
                ...
                try:
                    ...
                    logger.debug(f"Pillow version might not support anchor in textbbox: {e}. Using fallback.")
                    try:
                        tw = draw.textlength(no_text_msg, font=font)
                        th = self.font_size * 1.2
                    except AttributeError:
                        bbox_legacy = font.getbbox(no_text_msg) if hasattr(font, 'getbbox') else (0, 0, 50, 10)
                        tw = bbox_legacy[2] - bbox_legacy[0]; th = bbox_legacy[3] - bbox_legacy[1]
                    tx = (image.width - tw) / 2; ty = (image.height - th) / 2
                    bbox = (tx, ty, tx + tw, ty + th); text_anchor_nt = None; draw_pos = (tx, ty)
                bg_pad = 5
            ...
            except Exception as e: logging.error(f"Error rendering no-text msg: {e}", exc_info=True)
        else:
            for i, bbox_coords in enumerate(self.boxes):
                ...
                target_width = box_w * 1.5; target_height = box_h * 2.0
                current_font_size = self.font_size
                temp_font = font
                rendered_text = line; text_width = box_w; text_height = box_h
                while current_font_size >= 8:
                    try:
                        if self.font_path: temp_font = ImageFont.truetype(self.font_path, current_font_size)
                        else: temp_font = ImageFont.load_default(size=current_font_size)
                    except:
                        logger.warning(f"Failed to load font at size {current_font_size}. Using PIL default.")
                        temp_font = ImageFont.load_default(size=current_font_size)
                    avg_char_width_est = max(1, current_font_size * 0.6)
                    wrap_width = max(5, int(target_width / avg_char_width_est))
                    ...
                    try:
                        ...
                        current_text_width = text_bbox_calc[2] - text_bbox_calc[0]; current_text_height = text_bbox_calc[3] - text_bbox_calc[1]
                    except Exception as e:
                        logger.warning(f"textbbox failed during size check: {e}. Using estimate.")
                        current_text_width = target_width; current_text_height = current_font_size * len(wrapped_lines)
                    if current_text_width <= target_width and current_text_height <= target_height:
                        rendered_text = current_rendered_text; text_width = current_text_width; text_height = current_text_height
                        font = temp_font # Use the successfully sized font
                        break
                    current_font_size -= 1
                else:
                    logger.warning(f"Text wouldn't fit box {i} even at min size. Using smallest size.")
                    rendered_text = current_rendered_text; text_width = current_text_width; text_height = current_text_height
                    font = temp_font # Use the smallest tried font
                final_text_x = left + (box_w - text_width) / 2
                final_text_y = top + (box_h - text_height) / 2
                bg_pad_x, bg_pad_y = 3, 2
                bg_left = final_text_x - bg_pad_x; bg_top = final_text_y - bg_pad_y
                bg_right = final_text_x + text_width + bg_pad_x; bg_bottom = final_text_y + text_height + bg_pad_y
                bg_left = max(0, bg_left); bg_top = max(0, bg_top); bg_right = min(image.width, bg_right); bg_bottom = min(image.height, bg_bottom)
                try:
                     draw.rectangle([(bg_left, bg_top), (bg_right, bg_bottom)], fill=bg_outer_tuple)
                     draw.text((final_text_x, final_text_y), rendered_text, font=font, fill=font_color_tuple, spacing=4)
    ...
    def updateImageDisplay(self):
        ...
        try:
            ...
            if qimage.isNull(): logger.error("Failed to create QImage from rendered PIL image."); return
        ...
        if self.rendered_image and self.save_on_close:
            try:
                ...
                if parent_window and hasattr(parent_window, 'tray_icon') and parent_window.tray_icon and parent_window.tray_icon.isVisible():
                     ...
                     parent_window.tray_icon.showMessage("Save Error", f"Failed to save {failed_filename}.", QSystemTrayIcon.MessageIcon.Warning, 3000)
    ...
    def closeEvent(self, event):
        ...
        # REMOVED settings update from here
    ...
    def accept(self):
        ...
        # --- ADDED settings update logic here ---
        ...
        # --- End Added logic ---
        # closeEvent will still handle saving the image when the dialog actually closes
    ...
    def reject(self):
        ...
        # Settings are NOT updated on rejection by default.
        # If you wanted settings to be saved even on Esc/X,
        # you would add the settings update logic here too, similar to accept().
        super().reject() # Call the original reject method
    # MODIFIED __init__ signature
    def __init__(self, image_path, boxes, translated_lines, initial_font_size, target_language_code, control_window_ref, parent=None):
        # ... (initialization logic remains the same as the previous correction) ...
        super().__init__(parent)
        self.image_path = image_path
        self.boxes = boxes if boxes else []
        self.translated_lines = translated_lines if translated_lines else []
        ...
        if self.control_window_ref and hasattr(self.control_window_ref, 'last_viewer_settings'):
            # *** Fetch the LATEST settings from control window ***
            vs = self.control_window_ref.last_viewer_settings
            if vs: # Check if it's not empty
                logger.debug("Loaded viewer settings from ControlWindow reference.")
            else: # If empty in control window, fallback to loading file
                 settings_from_file = load_settings()
                 vs = settings_from_file.get('viewer_settings', {})
                 logger.debug("Loaded viewer settings from file as fallback (ControlWindow had no settings).")
        else: # Fallback if no control window ref
             settings_from_file = load_settings()
             vs = settings_from_file.get('viewer_settings', {})
             logger.warning("ControlWindow ref not available/attribute missing, loaded viewer settings from file.")
        ...
        # --- Use loaded settings (vs) with defaults ---
        default_font_color_tuple = (255, 255, 255, 255) # White default text
        default_bg_outer_tuple = (0, 0, 0, 200)     # Semi-transparent black default BG
        # Safely get tuple values, handling list or tuple from load ## REVISED ##
        font_color_val = vs.get('font_color', default_font_color_tuple)
        if isinstance(font_color_val, (list, tuple)) and len(font_color_val) == 4:
            font_color_tuple = tuple(font_color_val) # Ensure it's a tuple
    ...
    def initUI(self):
        # (initUI method remains the same)
        main_layout = QVBoxLayout(self)
        main_layout.setContentsMargins(10, 10, 10, 10)
        main_layout.setSpacing(10)
        self.image_label = QLabel(self)
    ...
    def resizeEvent(self, event):
        # (resizeEvent method remains the same)
        super().resizeEvent(event)
        self.updateImageDisplay()

    def updateFont(self, font_display_name):
        # (updateFont method remains the same)
        new_path = self.font_family_to_path.get(font_display_name)
        if new_path and os.path.exists(new_path):
            if self.font_path != new_path:
                 self.font_path = new_path
    ...
    def updateFontSize(self, value):
        # (updateFontSize method remains the same)
        if self.font_size != value:
             self.font_size = value
             self.font_size_value_label.setText(f"{value}pt")
             logger.debug(f"Viewer font size updated to: {self.font_size}")
    ...
    def _delayedRenderUpdate(self):
        # (_delayedRenderUpdate method remains the same)
        self.renderTranslatedImage()
        self.updateImageDisplay()

    def updateFontColor(self, color):
        # (updateFontColor method remains the same)
        if self.font_color != color:
            self.font_color = color
            logger.debug(f"Viewer font color updated to: {self.font_color.name(QColor.NameFormat.HexArgb)}")
            self.renderTranslatedImage()
    ...
    def updateBgColor(self, color):
        # (updateBgColor method remains the same)
        if self.bg_color_outer != color:
            self.bg_color_outer = color
            h, s, v, a = color.getHsvF()
            new_v_inner = max(0, v * 0.9)
    ...
    def resetStyles(self):
        # (resetStyles method remains the same)
        logger.debug("Resetting viewer styles.")
        default_font_color_tuple = (0, 0, 255, 255)
        default_bg_outer_tuple = (0, 0, 0, 180)
        ...
        parent_initial_size = 20
        # Check control_window_ref before accessing attributes
        if self.control_window_ref and hasattr(self.control_window_ref, 'default_font_size'):
            parent_initial_size = self.control_window_ref.default_font_size
    ...
    def renderTranslatedImage(self):
        # (renderTranslatedImage method remains the same)
        if not self.original_image:
             logger.error("renderTranslatedImage called with no original image loaded.")
             return
        image = self.original_image.copy()
        ...
        if not self.boxes:
            ...
            for i, bbox_coords in enumerate(self.boxes):
                ...
                box_h = bottom - top
                if box_w <= 0 or box_h <= 0: continue
                target_width = box_w * 1.5
                target_height = box_h * 2.0
                current_font_size = self.font_size
                temp_font = font
                rendered_text = line
                text_width = box_w
                ...
                while current_font_size >= 8:
                    ...
                    try:
                        ...
                        logger.warning(f"textbbox failed during size check: {e}. Using estimate.")
                        current_text_width = target_width
                        current_text_height = current_font_size * len(wrapped_lines)
                    if current_text_width <= target_width and current_text_height <= target_height:
    ...
    def updateImageDisplay(self):
        # (updateImageDisplay method remains the same)
        if not self.rendered_image or not self.image_label: return
        try:
            im = self.rendered_image
            if im.mode != "RGBA": im = im.convert("RGBA")
    ...
    def auto_save_image(self):
        # (auto_save_image method remains the same)
        save_path = None
        if self.rendered_image and self.save_on_close:
            try:
                timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
    ...
    def get_viewer_settings(self):
        # (get_viewer_settings method remains the same)
        return {
            'font_color': list(self.font_color.getRgb()),
            'bg_color_outer': list(self.bg_color_outer.getRgb()),
            'font_path': self.font_path,
    ...
    def get_geometry_settings(self):
        # (get_geometry_settings method remains the same)
        return { 'x': self.x(), 'y': self.y(), 'width': self.width(), 'height': self.height() }

    def load_viewer_geometry(self, settings):
        # (load_viewer_geometry method remains the same)
        if 'TranslatedImageViewer' in settings:
            try:
                geo = settings['TranslatedImageViewer']
                if all(k in geo for k in ('x', 'y', 'width', 'height')):
    ...
    def closeEvent(self, event):
        # (closeEvent method remains the same)
        logger.debug("TranslatedImageViewer closeEvent triggered.")
        if self.control_window_ref and hasattr(self.control_window_ref, 'update_last_viewer_settings'):
            current_settings = self.get_viewer_settings()
            self.control_window_ref.update_last_viewer_settings(current_settings)
        else:
            logger.warning("Could not update last viewer settings: ControlWindow reference missing.")
        self.auto_save_image()
        super().closeEvent(event)

    def accept(self):
        # (accept method remains the same)
        logger.debug("TranslatedImageViewer accepted (closed).")
        super().accept()

    def reject(self):
        # (reject method remains the same)
        logger.debug("TranslatedImageViewer rejected (closed).")
        super().reject()