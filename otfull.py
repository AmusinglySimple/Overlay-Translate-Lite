                ...
                if not ocr_result or not ocr_result[0]:
                    ...
                    if self.use_translate_with_ai and not self.live:
                        ...
                        if not translated_text or translated_text in ["No text detected.", original_text]:
                            logging.warning("AI translation failed or returned no text; falling back to Flask server.")
                            translated_text = None
                    ...
                    if translated_text is None:
                        ...
                        try:
                            ...
                            logging.error(f"Flask server translation failed: {e}")
...
class ControlWindow(QMainWindow):
    ...
    def selectSourceLanguage(self):
        ...
        dialog.setLabelText("Choose source language:")
        dialog.setComboBoxItems(["auto", "en", "es", "fr", "de", "it", "pt", "ru", "zh-cn", "ja", "ko"])
        dialog.setStyleSheet(GLOBAL_STYLESHEET)
        ok = dialog.exec()
        lang = dialog.textValue()
        if ok and lang:
            self.source_language = lang
            logging.info(f"Source language set to: {lang}")

    def selectTargetLanguage(self):
        dialog = QInputDialog(self)
        dialog.setWindowTitle("Select Target Language")
        dialog.setLabelText("Choose target language:")
        dialog.setComboBoxItems(["en", "es", "fr", "de", "it", "pt", "ru", "zh-cn", "ja", "ko"])
        dialog.setStyleSheet(GLOBAL_STYLESHEET)
        ok = dialog.exec()
        lang = dialog.textValue()
        if ok and lang:
            self.target_language = lang
            logging.info(f"Target language set to: {lang}")
    ...
    def start_flask_server(self):
        def run_flask():
            ...
            try:
                ...
                QMessageBox.critical(self, "Error", f"Failed to start Flask server: {e}")
            finally:
                self.flask_running = False
        ...
        # Wait until the server is up (or timeout after 5 seconds)
        ...
        logging.error("Flask server did not start within 5 seconds.")
        QMessageBox.critical(self, "Error", "Flask server failed to start within 5 seconds.")
    ...
    def cleanup(self):
        ...
        if self.flask_running:
            try:
                ...
                logging.info("Flask server shutdown request sent.")
            except requests.RequestException as e:
                logging.error(f"Failed to shutdown Flask server: {e}")
        
        # Wait for the Flask thread to finish
        if self.flask_thread and self.flask_thread.is_alive():
            self.flask_thread.join(timeout=5)
            if self.flask_thread.is_alive():
                logging.warning("Flask thread did not terminate within timeout.")
